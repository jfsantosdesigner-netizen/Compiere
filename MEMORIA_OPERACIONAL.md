...
| Item | Situação |
|---|---|
| Motor no PC | **v24** em `C:\CLAUDE\motor v24` (usa `C:\CLAUDE\MATERIAIS` direto). `motor v23` e `motor v13` = reserva. |
| Motor na nuvem | **v24** (GitHub `jfsantosdesigner-netizen/motor-cadernos-compiere`, ramo `main` = ramo `v24`). Instalada no PC em 25/09 (fim do dia). |
| Versões guardadas | Ramos `v9` … `v24` no GitHub (tags são bloqueadas pela conexão; por isso ramos). |
...
| Próximo | João testar a v24 no PC (arrastar a pasta no GERAR_CADERNO.bat de C:\CLAUDE\motor v24). Puxadores: descobrir como exportar do Promob. |
...
5. **Cada rodada de correções = nova versão**: atualizar `VERSAO.txt`, caminhos do `padrao.json` (`motor vN`), commit, ramo `vN` no GitHub. Próxima = **v25**.
...
- **DIVISÓRIA RIPADA** (6+ ripas ≤ 30 × 60–200 mm, ≥ 500 de comprimento, na mesma faixa de profundidade; ex.: ripado com painel de TV no meio do quarto): sai das paredes e vira **bloco próprio** no fim ("MÓDULOS E PAINÉIS – DIVISÓRIA RIPADA" + "MEDIDAS E ALTURAS – DIVISÓRIA RIPADA"). Junta bases, travessas, painel de TV e fechamento na parede. **Listagem: móvel SOZINHO (sem o ambiente), em DUAS pranchas: 3D FRONTAL (todas as ripas, como a cota) e 3D LATERAL RETA (90°) pelo lado da perna do L** (mostra as ripas do L; nunca diagonal), tabela completa nas duas, **um balão por tipo de peça**. **Cota: TODOS os vãos entre as ripas** (em cada faixa: embaixo, meio, em cima do painel; setas por dentro — o montador precisa da distância entre ripas), largura e altura totais, alturas das partes.
...
1. João testar a **v24** no PC e conferir na página; o que estiver errado vira condição/regra nova → v24.
